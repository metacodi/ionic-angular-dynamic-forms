...
export type stringOrExpr = string | {
    expr: string;
};
export type booleanOrExpr = boolean | {
    expr: string;
};
export type numberOrExpr = number | {
    expr: string;
};
/** @typeguard for `{ expr: string }` alternative of `stringOrExpr` | `booleanOrExpr` | `numberOrExpr`. */
export declare const isExprLiteral: (value: stringOrExpr | booleanOrExpr | numberOrExpr) => value is {
    expr: string;
};
