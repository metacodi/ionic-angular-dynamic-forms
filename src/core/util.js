 ...
 * ```
 */
export const evalExpr = (expr, options) => {
    if (!options) {
        options = {};
    }
    ...
    Object.keys(options.args).map(k => args[k] = options.args[k]);
    options.host = options.host || {};
    Object.keys(options.host).map(k => args[k] = options.host[k]);
...
export const evalCode = (code, args, host) => {
    if (!code) {
        return;
    }
    ...
    try {
        ...
        throw new Error(`Evaluating code '${code}'\n${e}\n\n> wrapper:\n${wrapper}`);
    }
    try {
        ...
        throw new Error(`Running code '${code}'\n${e}\n\n> wrapper:\n${wrapper}`);
    }
};
